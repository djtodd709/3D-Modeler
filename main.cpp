...
struct Object{
	...
	void save(){
		...
		for(list<Object>::const_iterator iterator = sceneGraph.begin(); iterator != sceneGraph.end(); iterator++){
			...
			for(int i=0; i<8; i++){
				for(int j=0; j<3; j++){
					savefile << o.bpvertices[i][j] << "\n";
				}
			}
			for(int i=0; i<6; i++){
				for(int j=0; j<4; j++){
					savefile << o.bpfaces[i][j] << "\n";
				}
			}
		}
		savefile.close();
		...
		if (savefile.is_open())
		{
			...
			while ( getline (savefile,objData) )
			{
				if(objData != ""){
					...
					for(int i=0; i<8; i++){
						o.bpvertices[i] = new float[3];
						for(int j=0; j<3; j++){
							getline(savefile,objData);
							o.bpvertices[i][j] = atof(objData.c_str());
						}
					}
					for(int i=0; i<6; i++){
						o.bpfaces[i] = new int[4];
						for(int j=0; j<4; j++){
							getline(savefile,objData);
							o.bpfaces[i][j] = atoi(objData.c_str());
						}
					}
					o.rayHit = false;
	...
	void rayCast(int x, int y){
		...
		gluUnProject(winX, winY, 0.0, matModelView, matProjection,
			...
			bool isSelected(Object o){
				if(abs(selectedObject->closestDist-o.closestDist)<0.00001f){
					if(abs(selectedObject->position[X]-o.position[X])<0.00001f){
						if(abs(selectedObject->position[Y]-o.position[Y])<0.00001f){
							if(abs(selectedObject->position[Z]-o.position[Z])<0.00001f){
								if(selectedObject->objType == o.objType){
									return true;
								}
							}
						}
			...
			void mouse(int btn, int state, int x, int y){
				...
				if (btn == GLUT_RIGHT_BUTTON){
					if (state == GLUT_DOWN){
						if(rayTest(x,y)){
							sceneGraph.remove_if(isSelected);
							selectedObject = &placeholder;
						}
					}
				}
